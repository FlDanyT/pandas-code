...
def WB_Mpstats_by_keywords():
    getDateSheet = getDataTable(config.ID_SHET_SETTING, [config.WB_NAME_SHEET_MPSTATS_BY_KEYWORDS])
    getDateSheet_List = getDataTable(config.ID_SHET_SETTING, [config.WB_NAME_SHEET_MPSTATS_BY_KEYWORDS_LIST])
    ...
    for number_row_date in range(lastRow_date):
        row_date = config.WB_NAME_SHEET_MPSTATS_BY_KEYWORDS_LIST + '!A' + str(number_row_date+2)
        valus_date = [['', '', '', '',]]
    ...
    for date_number in range(2):
      ...
      for number_row in range(lastRow):
        ...
        date_one = 'd1='+date
        date_two = '&d2='+date
        main_url_one = 'https://mpstats.io/api/wb/get/item/'
        ...
        if r.status_code == 200:           
            ...
            for word_values_list_row in word_values_list: # Получаем по очереди список
              ...
              if word_values_list_row in headers:
                ...
                try:
                  auto = value_colum['auto'][0][0]
                except:
                  auto = value_colum['auto'][0]

          
                row = config.WB_NAME_SHEET_MPSTATS_BY_KEYWORDS_LIST + '!A2'
                valus = [[date, get_artikul, word_values_list_row, pos]]
                appendSheets(config.ID_SHET_SETTING, row, valus)


if __name__ == "__main__":
    WB_Mpstats_by_keywords()